...
public abstract class ShipHandlerCommon {
    ...
    public void onChunkUpdate() {
        ...
        ship.setLayeredBlockVolumeCount(new int[chunk.maxY() - chunk.minY()]);
        for (int y = 0; y < ship.getLayeredBlockVolumeCount().length; y++) {
            for (int i = chunk.minX(); i < chunk.maxX(); i++) {
                for (int j = chunk.minZ(); j < chunk.maxZ(); j++) {
                    if (chunk.isBlockTakingWaterVolume(i, y + chunk.minY(), j)) {
                        int[] layeredBlockVolCount = ship.getLayeredBlockVolumeCount();
                        layeredBlockVolCount[y]++;
                        ship.setLayeredBlockVolumeCount(layeredBlockVolCount);
                    }
                }
            }
        }
